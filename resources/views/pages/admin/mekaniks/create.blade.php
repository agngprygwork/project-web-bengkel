{{-- resources/views/admin/mekaniks/create.blade.php --}}
@extends('layouts.app')

@section('title', 'Tambah Mekanik')
@section('page-title', 'Tambah Mekanik Baru')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
                <h3 class="text-white font-semibold">Form Tambah Mekanik</h3>
            </div>

            <form action="{{ route('admin.mekaniks.store') }}" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="spesialis" required
                            ...
                            @foreach ($spesialisOptions as $key => $value)
                                <option value="{{ $key }}" {{ old('spesialis') == $key ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                        @error('spesialis')
                    ...
                    <div>
                        ...
                        <input type="number" name="pengalaman_tahun" value="{{ old('pengalaman_tahun', 0) }}" required
                            min="0" max="50"
